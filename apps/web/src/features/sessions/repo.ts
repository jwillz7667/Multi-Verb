...
/**
 * Resolve a single `session_flags` row by id.
 *
 * The flagged-clip export needs to look up `(ts, session_id)` for one
 * flag without paginating the whole list. The caller MUST compare the
 * returned row's `sessionId` against the route's session id before
 * acting on it — a crafted `/exports/clips/{otherSessionsFlagId}` must
 * never bleed clips across sessions.
 *
 * Returns `null` for unknown ids — the route turns that into a 404 so
 * the response doesn't discriminate between "doesn't exist" and "exists
 * in another session/org" once tenancy lands.
 */
export async function findSessionFlagById(flagId: string): Promise<SessionFlagRow | null> {
  const row = await db.sessionFlag.findUnique({
    where: { id: flagId },
    select: {
      id: true,
      sessionId: true,
